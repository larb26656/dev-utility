import { ConversionOutput, ConversionError } from '@/types';
...
export function wrapConversion<TInput, TOutput>(
  fn: (input: TInput) => ConversionOutput<TOutput> | Promise<ConversionOutput<TOutput>>
) {
  return async (input: TInput): Promise<ConversionOutput<TOutput>> => {
    try {
      ...
      return createError(
        error instanceof Error ? error.message : 'An unknown error occurred during conversion'
      );
    }
  };
}
